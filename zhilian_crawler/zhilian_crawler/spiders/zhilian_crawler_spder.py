...
class ZhilianSpider(Spider):
    ...
    def parse(self,response):
        ...
        for job in job_total.find_all('table',attrs={'class':'newlist'})[1:]:
            item['company_name'] = job.find('td',attrs={'class':'gsmc'}).text.strip()
            item['salary'] = job.find('td',attrs={'class':'zwyx'}).text.strip()
            item['job_title'] = job.find('td',attrs={'class':'zwmc'}).text.strip()
